    ...
    if container.aliases.filter(alias_type=guardian_alias).exists():
        ...
        for transaction in all_transactions:
            if transaction['cod_ope']=='CONTRIBUTION':
                ...
                operations.create_spot_movement(container, source, target, details, transaction['des_mov'])
            elif transaction['cod_ope']=='BUY':
                security = SecurityContainer.objects.filter(aliases__alias_type=guardian_alias, aliases__alias_name=transaction['cod_tit'])
                if security.exists():
                    source = None
                    target = {'security': security[0], 'quantity': transaction['qta'], 'price': transaction['prezzo']}
                    details = {'trade_currency': transaction['cod_div_reg'],
                               'operation_date': transaction['data_ins'], 'trade_date': transaction['data_ope'], 'value_date': transaction['data_val'],
                               'spot_rate': transaction['cambiom'] if transaction.has_key('cambiom') and transaction['cambiom']!=None else (1.0/transaction['cambiod']),
                               'operation': 'BUY', 'impact_pnl': True
                               }
